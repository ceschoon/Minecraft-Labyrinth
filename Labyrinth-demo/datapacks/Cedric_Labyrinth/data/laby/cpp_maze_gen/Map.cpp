...
Map::Map(int mapHeight, int mapWidth, int seed)
{
	if (mapWidth%2==1)
	{
		m_mapWidth = mapWidth;
...
void Map::recursiveDivision(int recursionLevel)
{
	...
	else
	{
		...
		if (recursionLevel%2==0) 
		{
			Map map1(2*N+1,2*randIndex+1,rand());
			Map map2(2*N+1,2*(M-randIndex-1)+1,rand());
		...
		{
			Map map1(2*randIndex+1,2*M+1,rand());
			Map map2(2*(N-randIndex-1)+1,2*M+1,rand());
